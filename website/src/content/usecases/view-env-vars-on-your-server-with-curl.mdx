...
```bash
curl -X POST https://api.tune/api/v1/ingest -H "Authorization: Bearer YOUR_API_KEY" -H "Content-Type: application/json" -d '{ "name": "view environment variables", "avatar": "🔑", "content": "User john.doe viewed env vars on server" }'
```
